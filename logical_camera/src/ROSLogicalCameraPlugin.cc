...
#include "logical_camera/LogicalCameraImage.h"
#include "logical_camera/Model.h"
...
ROSLogicalCameraPlugin::~ROSLogicalCameraPlugin()
{
  ...
  std::string ROSLogicalCameraPlugin::DetermineModelType(const std::string& modelName)
  {
  ...
  std::string ROSLogicalCameraPlugin::DetermineModelId(const std::string& modelName)
  {
  ...
  std::string ROSLogicalCameraPlugin::TrimNamespace(const std::string& modelName)
  {
    // Trim namespaces
...
void ROSLogicalCameraPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->imagePub = this->rosnode->advertise<logical_camera::LogicalCameraImage>(imageTopic_ros, 1, false);
...
void ROSLogicalCameraPlugin::OnImage(ConstLogicalCameraImagePtr &_msg)
{
  ...
  logical_camera::LogicalCameraImage imageMsg;
  ...
  for (int i = 0; i < _msg->model_size(); ++i)
  {
    ...
    else
    {
      ...
      if (this->anonymizeModels)
      {
        modelNameToUse = "model_" + DetermineModelId(modelName);
        modelTypeToUse = "model";
      }
      else
      {
        modelNameToUse = TrimNamespace(modelName);
  ...
  const std::string & modelName, const std::string & modelType)
{
  ...
  if (this->onlyPublishKnownModels)
  {
    ...
    it = std::find(this->knownModelNames.begin(), this->knownModelNames.end(), TrimNamespace(modelName));
